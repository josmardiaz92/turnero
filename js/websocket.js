...
function iniciarWebsocket(){
	...
	socket = new WebSocket("ws://192.168.1.9:8888/php/proyectos/turnero/turnero/server.php");
...
function generate_table(table = null){
	...
	var th = "<tr><th>Turno</th><th colspan='2'>Consultorio</th></tr>";
	
	for(var i = 0; i < table.length; i++){	
	
		if(i == 0){
	
			tr = "<tr><td><span  class='primer-fila'>"+table[i]['turno']+"</span></td><td class='td-caja'><span class='caja primer-fila'>Consultorio</span></td><td class='no-caja'><span  class='primer-fila'>"+table[i]['Consultorio']+"</span></td></tr>".toString();
	
		}else{
	
			tr = tr+"<tr><td>"+table[i]['turno']+"</td><td class='td-caja'><span class='caja'>Consultorio</span></td><td class='no-caja'>"+table[i]['Consultorio']+"</td></tr>".toString();
